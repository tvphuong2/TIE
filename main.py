...
if __name__ == '__main__':
    ...
    if os.path.exists("errs") is False:
        os.mkdir("errs")
    ...
    print('num_ents, num_rels, num_time_steps-----------------------------------')
    print(num_ents, num_rels, num_time_steps)
    ...
    if args.load_base_model:
        experiment_path = os.getcwd()
        experiment_path = os.path.join(experiment_path, 'experiments/base-model')
        print(args.module)
        print(args.dataset)
        folder_path = args.module + '-' + args.dataset.split('/')[1] + '-' + args.score_function
        print(folder_path)
        experiment_path = os.path.join(experiment_path, folder_path)
        experiment_path = os.path.join(experiment_path, 'snapshot-0')
        files = os.listdir(experiment_path)
        try:
            files = files[0]
            experiment_path = os.path.join(experiment_path, files)
        except e:
            print(e)

        print('experiment path')
        print(experiment_path)
        base_model_path = experiment_path
        

        # base_model_path = glob.glob(os.path.join(args.base_model_path, "*.ckpt"))[0]
        # base_model_path = os.path.join(experiment_path, "experiments/DE-wiki-complex-patience-100-seed-123-end-time-step-54/version_202202170859/snapshot-0/_ckpt_epoch_29.ckpt")
        # print("base_model_path--------------------------------")
        # print(base_model_path)
        base_model_checkpoint = torch.load(base_model_path, map_location=lambda storage, loc: storage)
        print("load base model checkpont ok")
