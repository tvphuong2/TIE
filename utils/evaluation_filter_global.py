...
class EvaluationFilterGlobal:
    def __init__(self, model):
        ...
        self.num_ents = model.num_ents
        self.num_rels = model.num_rels
        self.calc_score = model.calc_score
        # print("Constructing eval filter")
    ...
    def get_true_subject_and_object_global(self):
        if os.path.exists(true_subject_path) and os.path.exists(true_object_path):
            with open(true_subject_path, "rb") as f:
                self.true_subject_global_dict = pickle.load(f)
            with open(true_object_path, "rb") as f:
                self.true_object_global_dict = pickle.load(f)
        else:
            ...
            with open(true_subject_path, 'wb') as fp:
                pickle.dump(self.true_subject_global_dict, fp)
            with open(true_object_path, 'wb') as fp:
                pickle.dump(self.true_object_global_dict, fp)

    def get_true_subject_and_object_val(self):
        true_subject_val_path = os.path.join(self.args.dataset, "true_subjects_val.pt")
        true_object_val_path = os.path.join(self.args.dataset, "true_objects_val.pt")
        true_subject_test_path = os.path.join(self.args.dataset, "true_subjects_test.pt")
        true_object_test_path = os.path.join(self.args.dataset, "true_objects_test.pt")

        if os.path.exists(true_subject_val_path) and os.path.exists(true_object_val_path)\
                and os.path.exists(true_subject_test_path) and os.path.exists(true_object_test_path):
            with open(true_subject_val_path, "rb") as f:
                self.true_subject_val_dict = pickle.load(f)
            with open(true_object_val_path, "rb") as f:
                self.true_object_val_dict = pickle.load(f)
            with open(true_subject_test_path, "rb") as f:
                self.true_subject_test_dict = pickle.load(f)
            with open(true_object_test_path, "rb") as f:
                self.true_object_test_dict = pickle.load(f)
        else:
            ...
            with open(true_subject_val_path, 'wb') as fp:
                pickle.dump(self.true_subject_val_dict, fp)
            with open(true_object_val_path, 'wb') as fp:
                pickle.dump(self.true_object_val_dict, fp)
            with open(true_subject_test_path, 'wb') as fp:
                pickle.dump(self.true_subject_test_dict, fp)
            with open(true_object_test_path, 'wb') as fp:
                pickle.dump(self.true_object_test_dict, fp)
    '''

    def get_true_subject_and_object_global(self):
        true_subject_path = os.path.join(self.args.dataset, "true_subjects.db")
        true_object_path = os.path.join(self.args.dataset, "true_objects.db")
        true_subject_val_path = os.path.join(self.args.dataset, "true_subjects_val.db")
        true_object_val_path = os.path.join(self.args.dataset, "true_objects_val.db")
        true_subject_test_path = os.path.join(self.args.dataset, "true_subjects_test.db")
        true_object_test_path = os.path.join(self.args.dataset, "true_objects_test.db")

        if os.path.exists(os.path.join(self.args.dataset, "true_subjects.db.dat")) and \
            os.path.exists(os.path.join(self.args.dataset, "true_objects.db.dat")) and \
            os.path.exists(os.path.join(self.args.dataset, "true_subjects_val.db.dat")) and \
            os.path.exists(os.path.join(self.args.dataset, "true_objects_val.db.dat")) and \
            os.path.exists(os.path.join(self.args.dataset, "true_subjects_test.db.dat")) and \
            os.path.exists(os.path.join(self.args.dataset, "true_objects_test.db.dat")):

            print("loading the evaluation shelve")
            self.true_subject_global_dict = shelve.open(true_subject_path)
            self.true_object_global_dict = shelve.open(true_object_path)
            self.true_subject_val_dict = shelve.open(true_subject_val_path)
            self.true_object_val_dict = shelve.open(true_object_val_path)
            self.true_subject_test_dict = shelve.open(true_subject_test_path)
            self.true_object_test_dict = shelve.open(true_object_test_path)
        else:
            self.true_subject_global_dict = shelve.open(true_subject_path)
            self.true_object_global_dict = shelve.open(true_object_path)
            self.true_subject_val_dict = shelve.open(true_subject_val_path)
            ...
            print("computing the evaluation shelve")
            for t in self.model.total_time:
                quads = torch.cat([self.model.time2quads_train[t], self.model.time2quads_val[t], self.model.time2quads_test[t]])
                val_quads = self.model.time2quads_val[t]
                test_quads = self.model.time2quads_test[t]
                true_subject_dict, true_object_dict = get_true_subject_and_object_per_graph(quads[:, :3])
                true_subject_dict_val, true_object_dict_val = get_true_subject_and_object_per_graph(val_quads[:, :3])
                true_subject_dict_test, true_object_dict_test = get_true_subject_and_object_per_graph(test_quads[:, :3])

                write_to_shelve(self.true_subject_global_dict, true_subject_dict, t)
                write_to_shelve(self.true_object_global_dict, true_object_dict, t)
                write_to_shelve(self.true_subject_val_dict, true_subject_dict_val, t)
                write_to_shelve(self.true_object_val_dict, true_object_dict_val, t)
                write_to_shelve(self.true_subject_test_dict, true_subject_dict_test, t)
                write_to_shelve(self.true_object_test_dict, true_object_dict_test, t)
    ...
    def calc_relative_rank(self, quadruples, known_entities, global2known_func, test_set):
        ...
        for i in range(len(quadruples)):
            ...
            s_r_key = "{}+{}+{}".format(t, s, r)
            o_r_key = "{}+{}+{}".format(t, o, r)
            for prev_t in range(max(0, t - self.args.eval_seq_len), t):
                prev_s_r_key = "{}+{}+{}".format(prev_t, s, r)
                prev_o_r_key = "{}+{}+{}".format(prev_t, o, r)

                if prev_s_r_key in self.true_object_global_dict.keys():
                    prev_true_object += self.true_object_global_dict[prev_s_r_key].tolist()

                if prev_o_r_key in self.true_subject_global_dict.keys():
                    prev_true_subject += self.true_subject_global_dict["{}+{}+{}".format(prev_t, o, r)].tolist()
                # if s in self.true_object_global_dict[s_prev_t] and r in self.true_object_global_dict[s_prev_t][s]:
                # if o in self.true_subject_global_dict[s_prev_t] and r in self.true_subject_global_dict[s_prev_t][o]:

            # true subject or true object: global -> known
            if (s, r) not in subject_relation_dict:
                subject_relation_dict[(s, r)] = 0
                cur_true_object_idx = global2known_func(true_object_dict[s_r_key])
                all_cur_true_object_idx = global2known_func(self.true_object_global_dict[s_r_key])
                cur_object_rank = object_rank[i].unsqueeze(0).cpu()
                positive_object_rank = torch.nonzero(
                    cur_object_rank.expand(len(cur_true_object_idx), rank_length) == torch.tensor(
                        cur_true_object_idx).view(-1, 1))[:, 1]
                raw_ranks.append(positive_object_rank)
                self.calc_relative_rank_per_query_type(first_positive_ranks, relative_ranks, deleted_facts_ranks,
                                                       both_positive_ranks, prev_true_object,
                                                       positive_object_rank, cur_true_object_idx, cur_object_rank,
                                                       all_cur_true_object_idx, rank_length, global2known_func)

            if (o, r) not in object_relation_dict:
                object_relation_dict[(o, r)] = 0
                cur_true_subject_idx = global2known_func(true_subject_dict[o_r_key])
                all_cur_true_subject_idx = global2known_func(self.true_subject_global_dict[o_r_key])
                cur_subject_rank = subject_rank[i].unsqueeze(0).cpu()
                positive_subject_rank = torch.nonzero(
                    cur_subject_rank.expand(len(cur_true_subject_idx), rank_length) == torch.tensor(
                        cur_true_subject_idx).view(-1, 1))[:, 1]
                raw_ranks.append(positive_subject_rank)
                self.calc_relative_rank_per_query_type(first_positive_ranks, relative_ranks, deleted_facts_ranks,
                                                       both_positive_ranks, prev_true_subject,
                                                       positive_subject_rank, cur_true_subject_idx, cur_subject_rank,
                                                       all_cur_true_subject_idx, rank_length, global2known_func)
    ...
    def mask_eval_set(self, quadruples, global2known_func, entity_length, calc_mask, mode='tail'):
        ...
        for i in range(test_size):
            ...
            if mode == 'tail':
                tails = self.true_object_global_dict["{}+{}+{}".format(t, s, r)]
                tail_idx = global2known_func(tails)
                mask[i][tail_idx] = 1
                mask[i][global2known_func(o)] = 0
            elif mode == 'head':
                heads = self.true_subject_global_dict["{}+{}+{}".format(t, o, r)]
    ...
    def perturb_and_get_rank(self, anchor_entities, relation_embedding, target, time_tensor,
        ...
        if self.args.use_cuda and 'wikidata' not in self.args.dataset:
    ...
    def get_unmasked_score(self, anchor_entities, relation_embedding, time_tensor, known_entities, mode ='tail'):
        ...
        if 'wikidata' in self.args.dataset:
            ...
            relation_embedding = relation_embedding.cpu()
        if mode == 'tail':
        ...
        if self.args.score_function == 'convKB':
            print(self.args)

            '--------------------------------'
            return self.calc_score(subject_embedding, relation_embedding, object_embedding, self.args.hidden_size ,self.num_ents, self.num_rels)
        else: 
