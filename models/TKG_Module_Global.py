...
class TKG_Module_Global(LightningModule):
    def __init__(self, args, num_ents, num_rels):
        ...
        self.calc_score = {'distmult': distmult, 'complex': complex, 'transE': transE, 'atise':ATiSE_score, 'convKB': ConvKB, 'rotate': rotate}[args.score_function]
        self.build_model()
    ...
    def test_epoch_end(self, outputs):
        ...
        if not self.args.train_base_model:
            ...
            for t in range(self.args.start_time_step, self.args.end_time_step):
                ...
                self.metrics_collector.update_diff_time_eval_results(self.time, t, mrr.cpu(), hit_1.cpu(), hit_3.cpu(), hit_10.cpu())
    ...
    def train_link_prediction(self, subject_embedding, relation_embedding, object_embedding, labels, corrupt_tail=True):
        ...
        if self.args.score_function == 'convKB':
            ...
            '--------------------------------'
            score = self.calc_score(subject_embedding, relation_embedding, object_embedding, self.hidden_size ,self.num_ents, self.num_rels)
    ...
    def link_classification_loss(self, ent_embed, rel_embeds, triplets, labels):
        ...
        if self.args.score_function == 'convKB':
            score = self.calc_score(s, r, o, self.hidden_size ,self.num_ents, self.num_rels)
